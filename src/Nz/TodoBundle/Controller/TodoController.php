...
class TodoController extends FOSRestController
{
     ...
     * )
     * 
     * @Annotations\View(template = "NzTodoBundle:Todo:createTodo.html.twig")
     */
     ...
     *
     * @Annotations\View(
     *   template = "NzTodoBundle:Todo:createTodo.html.twig",
    ...
    public function postTodosAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            /* return new \Symfony\Component\HttpFoundation\JsonResponse(['id' => $todo->getId()], Codes::HTTP_OK); */
            /*return View::createRouteRedirect('get_todos', ['id' => $todo->getId()], Codes::HTTP_OK);*/
            return $this->routeRedirectView('get_todos', ['id' => $todo->getId()]);
        }
    ...
    {
        /*
          $edit_todo = $request->request->get('todo');
          unset($edit_todo['id']);
          $request->request->set('todo', $edit_todo);
         */
